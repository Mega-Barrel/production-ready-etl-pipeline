...
class TestS3BucketConnectorMethods(unittest.TestCase):
    """
    Testing the S3 bucketConnector class
    """
    ...
    def setUp(self):
        ...
        self.s3_endpoint_url = 'https://s3.eu-central-1.amazon.aws.com'
        self.s3_bucket_name = 'test-bucket'
        # Creating s3 access keys as environment variables
        os.environ[self.s3_access_key] = 'KEY1'
        os.environ[self.s3_secret_key] = 'KEY2'
        # Create the bucket on mocked S3
        self.s3 = boto3.resource(service_name='s3', endpoing_url=self.s3_endpoint_url)
    ...
    def test_write_df_to_s3_empty(self):
        ...
        # Test init
        def_empty = pd.DataFrame()
        key = 'key.csv'
        file_format = 'csv'
        # Method execution
        with self.assertLogs() as logm:
            result = self.s3_bucket_conn.write_df_to_s3(def_empty, key, file_format)
            # Log test after method execution
            self.assertIn(log_exp, logm.output[0])
        # Test after method execution
