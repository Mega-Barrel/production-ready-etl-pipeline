...
class XetraETL():
    ...
    def transform_report1(self, data_frame: pd.DataFrame):
        ...
        data_frame[self.trg_args.tar_col_clos_price] = data_frame\
    ...
    def load(self, data_frame: pd.Data_Frame):
        """
        ...
        # Writing to target
        self.s3_bucket_target.write_df_to_s3(data_frame, target_key, self.trg_args.trg_format)
        self._logger.info('Xetra target data successfully written.')
        # Updating the meta file
        MetaProcess.update_meta_file(self.meta_update_list, self.meta_key, self.s3_bucket_target)
